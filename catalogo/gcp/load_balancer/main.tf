...
module "external" {
  ...
  tcp_ports             = [for p in try(each.value.tcp_ports, []) : tostring(p)]
  udp_ports             = [for p in try(each.value.udp_ports, []) : tostring(p)]
