...
variable "root_password" {
  description = "Password for the root user; when null and generate_password is false no root password is set by Terraform."
  type        = string
  default     = null
  sensitive   = true
}
...
variable "generate_password" {
  description = "When true, the module generates a strong root password using the random provider. Ignored if root_password is set."
  type        = bool
  default     = false
}
...
variable "database_charset" {
  description = "Default charset for the database created via database_name."
  type        = string
  default     = "utf8mb4"
}

variable "database_collation" {
  description = "Default collation for the database created via database_name."
  type        = string
...
variable "additional_users" {
  description = "Extra Cloud SQL users to create alongside root. Pass passwords through Terraform variables marked sensitive."
  type = list(object({
    name     = string
    password = string
